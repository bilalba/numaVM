...
│       ├── deploymagi-auth.service
│       ├── deploymagi-control-plane.service
│       ├── deploymagi-dashboard.service
│       └── deploymagi-admin.service
...
**Services** are managed via systemd: `deploymagi-auth`, `deploymagi-control-plane`, `deploymagi-dashboard`, `deploymagi-admin`. Unit files in `infra/systemd/`. All services auto-restart on failure (`RestartSec=5`). Logs via `journalctl -u deploymagi-auth` etc.
