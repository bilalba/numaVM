...
./deploy.sh --skip-build       # Skip dashboard build
...
**Services** are managed via systemd: `deploymagi-auth`, `deploymagi-control-plane`, `deploymagi-dashboard`. Unit files in `infra/systemd/`. All services auto-restart on failure (`RestartSec=5`). Logs via `journalctl -u deploymagi-auth` etc.
